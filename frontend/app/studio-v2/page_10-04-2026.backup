...
export default function StudioV2Page() {
        ...
        if (sizeAttr) {
          ...
          if (!seenSizes.has(sVal)) {
            ...
            if (dimMatch) {
              ...
              if (target === 'landscape' && h > w) isValid = false;
              if (target === 'square' && Math.abs(w - h) > 1) isValid = false;
              ...
              if (isValid) {
                ...
                variantSizes.push({
                  id: `vs-${sVal.replace(/\s/g, '')}`, label: sVal,
                  width: w, height: h,
  ...
  useEffect(() => {
    ...
    if (activeCat?.categoryAttributes) {
      ...
      if (catSizeAttr?.attributeOptions) {
        ...
        if (!selectedSize) setSelectedSize(parsed[0]);
      }
    }
  ...
  useEffect(() => {
    if (selectedDesign?.productId && selectedDesign.productId !== activeProductId) {
      ...
      // Reset selected size when product changes so it picks the default size of the new product
  ...
  const handleAddToCart = async () => {
    ...
    if (!selectedSize) return alert("Please select a frame size.");
  ...
  const pushQuery = (newShape: string | null | undefined, newDesignId?: string | null | undefined, newFrameCount?: number, singleEditIdx?: number) => {
    ...
    if (newFrameCount) url += `&frameCount=${newFrameCount}`;
    ...
    return (
      <div className="bg-white min-h-screen">
        <main className="max-w-[900px] mx-auto px-4 md:px-6 py-6">
          ...
          <div className="bg-[#E7F3FB] border border-[#d6ebfa] rounded-lg flex shadow-sm mb-8 overflow-hidden">
            <div className="flex items-center gap-2 font-black text-[#212529] text-sm tracking-wide px-5 py-3.5 uppercase border-r border-[#c5dff5]">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="4" cy="12" r="2" /><circle cx="12" cy="12" r="2" /><circle cx="20" cy="12" r="2" /></svg>
              FILTERS
            </div>
            <div className="flex flex-1">
              <button
                onClick={() => setDesignFilter('all')}
                className={`flex items-center justify-center gap-2 flex-1 font-bold text-sm transition-colors border-r border-[#c5dff5] py-3.5 ${designFilter === 'all' ? 'bg-[#1877F2] text-white' : 'bg-white text-gray-500 hover:bg-blue-50'
                  }`}
              >
                <ImageIcon size={16} /> All
              </button>
              <button
                onClick={() => setDesignFilter('popular')}
                className={`flex items-center justify-center gap-2 flex-1 font-bold text-sm transition-colors py-3.5 ${designFilter === 'popular' ? 'bg-[#1877F2] text-white' : 'bg-[#F8F9FA] text-gray-500 hover:bg-white'
                  }`}
              >
                <Star size={16} fill={designFilter === 'popular' ? 'white' : 'none'} /> Popular
              </button>
            </div>
          </div>

          {/* Design Cards — large, full-width, immersive */}
          {loading ? (
            ...
            <div className="flex flex-col gap-0">
              {filteredDesigns.map((d, idx) => (
                <div
                  ...
                  className="group cursor-pointer relative overflow-hidden border-b border-slate-100 last:border-b-0 hover:bg-blue-50/30 transition-colors"
                >
                  {/* Large immersive card — lifestyle background + centered product */}
                  <div className="relative w-full" style={{ aspectRatio: '16/9', maxHeight: '480px' }}>
                    {/* Lifestyle background */}
                    <div className="absolute inset-0 bg-[#c8cfda]">
                      {d.previewImage ? (
                        <img
                    ...
                    {/* Centered product frame with glossy acrylic effect */}
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div
                        className="relative shadow-[0_20px_60px_rgba(0,0,0,0.3)] group-hover:shadow-[0_30px_80px_rgba(0,0,0,0.4)] transition-all duration-700 group-hover:scale-[1.02] bg-white"
                        style={{
                          width: selectedShape.toLowerCase().includes('land') ? '55%' : selectedShape.toLowerCase().includes('port') ? '38%' : '42%',
                      ...
                      >
                        ...
                        <div className="absolute inset-0 bg-gradient-to-br from-white/40 via-white/10 to-transparent z-10 pointer-events-none" />
                        <div className="absolute top-0 left-0 right-0 h-1/3 bg-gradient-to-b from-white/30 to-transparent z-10 pointer-events-none" />
                        ...
                        {d.previewImage ? (
                          ...
                          <div className="w-full h-full bg-slate-100 flex flex-col items-center justify-center gap-3">
                            ...
                            <p className="text-slate-400 font-bold text-sm text-center leading-tight">Upload<br />Your Photo</p>
                          </div>
                        )}
                      </div>
                    </div>

                    {/* Flower decorative at bottom-left corner */}
                    <div className="absolute bottom-0 left-0 w-1/4 pointer-events-none opacity-90">
                      <img
                        src="https://images.unsplash.com/photo-1490750967868-88df5691cc25?auto=format&fit=crop&q=80&w=400"
                        className="w-full h-[120px] object-cover rounded-tr-full"
                        alt=""
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between px-6 py-4 bg-white">
                    <span className="text-sm font-bold text-slate-500">{d.name || `Design ${idx + 1}`}</span>
                    <span className="text-[13px] font-black text-[#1877F2] uppercase tracking-widest flex items-center gap-1.5 group-hover:gap-3 transition-all">
                      START DESIGN <ChevronRight size={16} strokeWidth={3} />
                    </span>
                  </div>
                </div>
              ))}
  ...
  const isCustomProduct = designIdParam || (categoryParam && !categoryParam.includes('gallery'));
  
  if (isCustomProduct && !singleEditIndex) {
        ...
        <main className="flex-1 overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row h-full">
            ...
            <div className="flex-1 relative flex items-center justify-center p-8 md:p-12 lg:p-16 bg-[#f8f9fa] border-r border-slate-100 min-h-0">
              <div className="relative group">
                ...
                <div className="relative bg-[#f0f0f0] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)] rounded-[2px] border border-slate-300 overflow-hidden">
                  <div
                    ...
                    onMouseDown={(e) => {
                      if (!uploadedPhotos[0]) return;
                      e.preventDefault();
                      const startX = e.clientX - (uploadedPhotos[0]?.x || 0);
                      const startY = e.clientY - (uploadedPhotos[0]?.y || 0);
                      const onMove = (me: MouseEvent) => {
                        setUploadedPhotos(prev => ({
                          ...prev,
                          [0]: { ...(prev[0] || { url: '', scale: 1, rotate: 0 }), x: me.clientX - startX, y: me.clientY - startY }
                        }));
                      };
                      const onUp = () => {
                    ...
                    onTouchStart={(e) => {
                      ...
                      const startX = touch.clientX - (uploadedPhotos[0]?.x || 0);
                      const startY = touch.clientY - (uploadedPhotos[0]?.y || 0);
                      const onMove = (te: TouchEvent) => {
                        const t = te.touches[0];
                        setUploadedPhotos(prev => ({
                          ...prev,
                          [0]: { ...(prev[0] || { url: '', scale: 1, rotate: 0 }), x: t.clientX - startX, y: t.clientY - startY }
                        }));
                      };
                      const onEnd = () => {
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-6 bg-gradient-to-t from-black/60 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-500 flex justify-center">
                    <label className="bg-white px-6 py-2.5 rounded-xl font-black text-[11px] uppercase tracking-widest flex items-center gap-2 cursor-pointer hover:bg-slate-50 active:scale-95 transition-all shadow-xl">
                      <CloudUpload size={16} /> Change Photo
              ...
              {/* ZOOM CONTROLS (IF PHOTO EXISTS) */}
              {hasPhoto && (
                <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex items-center gap-6 bg-white/90 backdrop-blur-md px-6 py-3 rounded-2xl border border-white shadow-2xl">
                  <button onClick={() => {
                    const s = (uploadedPhotos[0]?.scale || 1) - 0.1;
                    if (s >= 0.5) setUploadedPhotos(prev => ({ ...prev, [0]: { ...prev[0], scale: s } }));
                  }} className="text-slate-400 hover:text-blue-500"><ZoomOut size={20} /></button>
                  <div className="w-32 h-1 bg-slate-100 rounded-full relative overflow-hidden">
                    <div className="absolute inset-y-0 left-0 bg-blue-500" style={{ width: `${((uploadedPhotos[0]?.scale || 1) / 3) * 100}%` }} />
                  </div>
                  <button onClick={() => {
                    const s = (uploadedPhotos[0]?.scale || 1) + 0.1;
                    if (s <= 3) setUploadedPhotos(prev => ({ ...prev, [0]: { ...prev[0], scale: s } }));
                  }} className="text-slate-400 hover:text-blue-500"><ZoomIn size={20} /></button>
                </div>
              )}
            </div>
        ...
        <div className="bg-white rounded-[2rem] w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden shadow-2xl">
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex-[1.2] bg-[#efefef] flex items-center justify-center overflow-hidden relative">
              <div className="bg-white shadow-2xl relative overflow-hidden" style={{
                width: '400px',
                height: '400px',
                borderRadius: selectedShape.toLowerCase().includes('circle') ? '50%' : '4px',
                ...
                {selectedShape.toLowerCase().includes('heart') && selectedBorderColor !== 'none' && (
                  <svg className="absolute inset-0 w-full h-full pointer-events-none z-10" viewBox="0 0 100 100" preserveAspectRatio="none">
                    <path
                      d="M50,20 C45,5 35,0 25,0 C10,0 0,10 0,30 C0,60 50,100 50,100 C50,100 100,60 100,30 C100,10 90,0 75,0 C65,0 55,5 50,20 Z"
                      fill="none"
                ...
                {selectedShape.toLowerCase().includes('heart') === false && selectedBorderColor !== 'none' && (
                  <div className="absolute inset-0 pointer-events-none z-10" style={{ border: `8px solid ${selectedBorderColor}`, borderRadius: selectedShape.toLowerCase().includes('circle') ? '50%' : '0' }} />
                )}
                {workingPhoto.url ? (
                  <div
                    className="w-full h-full cursor-move relative"
                    onMouseDown={(e) => {
                      const startX = e.clientX - workingPhoto.x;
                      const startY = e.clientY - workingPhoto.y;
                      const onMove = (me: MouseEvent) => {
                        setWorkingPhoto(p => p ? { ...p, x: me.clientX - startX, y: me.clientY - startY } : null);
                      };
                      const onUp = () => {
                        window.removeEventListener('mousemove', onMove);
                        window.removeEventListener('mouseup', onUp);
                  ...
                  >
                    <img
                      id="editing-photo-ref"
                      src={resolveMedia(workingPhoto.url, API_URL)}
                      className="pointer-events-none absolute max-w-none"
                      style={{
                        transform: `translate(${workingPhoto.x}px, ${workingPhoto.y}px) scale(${workingPhoto.scale}) rotate(${workingPhoto.rotate}deg)`,
                        transition: 'transform 0.05s linear'
                      }}
                      onLoad={(e: any) => {
                        const { naturalWidth, naturalHeight } = e.target;
                        const scale = Math.max(400 / naturalWidth, 400 / naturalHeight);
                        setWorkingPhoto(p => p && p.scale === 1 ? { ...p, scale } : p);
                      }}
                    />
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                      <div className="bg-black/20 text-white p-2 rounded-full backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity">
                        <Maximize2 size={24} />
                      </div>
                    </div>
                    <div className="absolute inset-0 border-2 border-[#1877F2] pointer-events-none shadow-[0_0_0_1000px_rgba(239,239,239,0.5)]" />
                  </div>
                ) : (
  ...
  if (categoryParam.includes('gallery') && shapeParam && designIdParam && !searchParams.get('singleEdit')) {
    ...
    return (
      <div className="bg-[#f2f2f2] min-h-screen flex flex-col overflow-hidden font-sans">
        ...
        <div className="flex justify-center py-4 gap-2 z-40">
          ...
          <span className="bg-[#d6ebfa] text-[#1877F2] text-[11px] font-black px-4 py-2 rounded-xl uppercase">No Border</span>
        </div>

        {/* Main Frames List */}
        <main className="flex-1 overflow-x-auto no-scrollbar scroll-smooth pb-64">
          <div className="flex gap-16 px-16 min-w-max h-full items-start pt-8">
            {[...Array(totalFrames)].map((_, i) => {
              const photo = uploadedPhotos[i];
              return (
                <div key={i} className="flex flex-col items-center">
                  <div className="w-[320px] md:w-[380px] bg-white rounded-[40px] shadow-[0_20px_60px_rgba(0,0,0,0.06)] border border-gray-100 p-10 flex flex-col relative transition-all duration-500 hover:shadow-[0_40px_100px_rgba(0,0,0,0.1)]">
                    {/* Dimension markers */}
                    ...
                    <div className="absolute right-6 top-1/2 -translate-y-1/2 flex flex-col items-center gap-3 h-[60%] text-slate-300">
                      ...
                      <div className="w-[2px] flex-1 bg-slate-100" />
                    ...
                    <button 
                      ...
                      className="absolute top-6 right-8 text-slate-200 p-2 hover:bg-red-50 hover:text-red-500 rounded-2xl transition-all"
                    >
                      <Trash2 size={18} strokeWidth={2.5} />
                    </button>

                    <div className="bg-white shadow-2xl relative overflow-hidden mt-6 group/photo aspect-square" style={{
                      borderRadius: selectedShape.toLowerCase().includes('circle') ? '50%' : '8px',
                      clipPath: selectedShape.toLowerCase().includes('heart') ? 'url(#heart-clip)' : (selectedShape.toLowerCase().includes('hex') ? 'url(#hexagon-clip)' : 'none')
                    }}>
                      {photo ? (
                        <img src={resolveMedia(photo.url, API_URL)} className="w-full h-full object-cover" alt={`Frame ${i+1}`} />
                      ) : (
                      ...
                      {!photo && (
                         <div className="absolute inset-0 flex items-center justify-center">
                            <label className="bg-white text-slate-900 w-12 h-12 rounded-full flex items-center justify-center shadow-2xl cursor-pointer hover:scale-110 active:scale-95 transition-all">
                    ...
                    <div className="mt-10 flex gap-3">
                      <button 
                        ...
                        className="flex-[2] h-12 bg-gray-50 hover:bg-white text-slate-600 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 border border-slate-100"
                      >
                        <ImageIcon size={14} /> Edit
                      </button>
                      <button className="flex-1 h-12 bg-gray-50 hover:bg-white text-slate-400 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all border border-slate-100">
                        Copy
                      </button>
                    </div>
                  </div>
                  <span className="mt-8 text-[11px] font-black uppercase text-slate-400 tracking-widest bg-white px-6 py-2 rounded-full shadow-sm border border-slate-100">Frame {i + 1} of {totalFrames}</span>
                </div>
              );
            })}
          </div>
        ...
        {/* Floating Action Card - PrintShoppy Style */}
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[150] w-[90%] max-w-2xl px-4">
           <div className="bg-white rounded-[40px] shadow-[0_30px_80px_rgba(0,0,0,0.2)] overflow-hidden border border-slate-100">
              <div className="p-8 flex flex-col md:flex-row items-center gap-8">
                 <div className="flex-1">
                    <div className="flex items-center gap-3 mb-1 text-slate-800">
                       <span className="text-xl font-black uppercase">Frames Count</span>
                       <span className="text-3xl font-black text-[#1caf9c]">{totalFrames}</span>
                    </div>
                    <p className="text-[10px] font-black uppercase tracking-widest text-[#1caf9c]">EACH FRAME ₹{displayPrice}/-</p>
                 </div>
                 <div className="flex items-center gap-6">
                    <div className="flex flex-col items-end">
                       <span className="text-xs text-slate-300 font-bold line-through">₹{originalPrice}</span>
                       <span className="text-4xl font-black text-[#1877F2] tracking-tighter">₹{totalPrice}</span>
                       <span className="text-[10px] font-black text-[#1caf9c] uppercase tracking-widest mt-1">₹{savings} SAVE</span>
                    </div>
                 </div>
              </div>

              <label className="w-full h-14 bg-[#f4fffd] hover:bg-[#ebfaf7] text-[#1caf9c] font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-2 transition-all border-t border-slate-50 cursor-pointer">
              ...
              <div className="bg-slate-50/80 p-4 flex gap-4 border-t border-gray-100">
                 <button onClick={() => setActiveGalleryOverlay('size')} className="flex-1 bg-white h-20 rounded-[28px] flex flex-col items-center justify-center gap-1.5 hover:shadow-lg transition-all border border-slate-100 group">
                    <Maximize2 size={18} className="text-slate-400 group-hover:text-blue-500" />
                    <span className="text-[10px] font-black uppercase text-slate-500">Size</span>
                 </button>
                 <button onClick={() => setActiveGalleryOverlay('shape')} className="flex-1 bg-white h-20 rounded-[28px] flex flex-col items-center justify-center gap-1.5 hover:shadow-lg transition-all border border-slate-100 group">
                    <LayoutGrid size={18} className="text-slate-400 group-hover:text-blue-500" />
                    <span className="text-[10px] font-black uppercase text-slate-500">Shape</span>
                 </button>
                 <button onClick={() => setActiveGalleryOverlay('border')} className="flex-1 bg-white h-20 rounded-[28px] flex flex-col items-center justify-center gap-1.5 hover:shadow-lg transition-all border border-slate-100 group">
                    <Square size={18} className="text-slate-400 group-hover:text-blue-500" />
                    <span className="text-[10px] font-black uppercase text-slate-500">Border</span>
                 </button>
                 <button 
                  onClick={handleAddToCart}
                  disabled={isCapturing}
                  className="flex-[4] bg-[#1877F2] h-20 rounded-[28px] flex items-center justify-center gap-4 text-white font-black uppercase text-xl shadow-2xl active:scale-[0.97] transition-all disabled:opacity-50"
                 >
                    {isCapturing ? <Loader2 size={24} className="animate-spin" /> : <>Add to Cart <ArrowRight size={24} /></>}
                 </button>
              </div>
           </div>
        </div>
          ...
          <div className="lg:col-span-6 bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col pt-4 overflow-hidden h-[450px] md:h-[550px]">
            ...
            <div className="flex-1 bg-[#FAF9F6] flex items-center justify-center overflow-hidden relative">
              <div className="transition-transform duration-500 ease-out" style={{ transform: `scale(${zoom * 0.85})` }}>
                <div id="studio-canvas-capture" className="shadow-2xl border-8 border-white ring-1 ring-gray-200 bg-white" style={{
                  width: `${canvasW * 25}px`,
                  height: `${canvasH * 25}px`,
                  borderRadius: selectedShape.toLowerCase().includes('heart') ? '50%' : '4px',
                  overflow: 'hidden'
                }}>
                  {selectedDesign && (
                    <DesignCanvas
      ...
      <svg width="0" height="0" style={{ position: 'absolute' }}>
        <defs>
          <clipPath id="heart-clip" clipPathUnits="objectBoundingBox">
            <path d="M0.5,0.2 C0.45,0.05 0.35,0 0.25,0 C0.1,0 0,0.1 0,0.3 C0,0.6 0.5,1 0.5,1 C0.5,1 1,0.6 1,0.3 C1,0.1 0.9,0 0.75,0 C0.65,0 0.55,0.05 0.5,0.2 Z" />
          </clipPath>
          <clipPath id="hexagon-clip" clipPathUnits="objectBoundingBox">
            <path d="M0.5,0 L0.95,0.25 L0.95,0.75 L0.5,1 L0.05,0.75 L0.05,0.25 Z" />
          </clipPath>
  ...
  async function handlePhotoUpload(idx: number, urlOrFile: string | File, init?: Partial<PhotoState>) {
    if (!urlOrFile) {
